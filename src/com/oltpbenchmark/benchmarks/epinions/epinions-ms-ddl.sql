-- SQL Server epinions DDL

IF NOT EXISTS ( SELECT  *
                FROM    sys.objects
                WHERE   name = 'user' ) 
begin 
CREATE TABLE "user" (
  u_id int NOT NULL,
  name varchar(128) DEFAULT NULL,
  PRIMARY KEY (u_id)
)
end;

IF NOT EXISTS ( SELECT  *
                FROM    sys.objects
                WHERE   name = 'item' ) 
begin 
CREATE TABLE "item" (
  i_id int NOT NULL,
  title varchar(20) DEFAULT NULL,
  PRIMARY KEY (i_id)
)
end;

IF NOT EXISTS ( SELECT  *
                FROM    sys.objects
                WHERE   name = 'review' ) 
begin 
CREATE TABLE "review" (
  a_id int NOT NULL,
  u_id int NOT NULL REFERENCES "user" (u_id),
  i_id int NOT NULL REFERENCES "item" (i_id),
  rating int DEFAULT NULL,
  rank int DEFAULT NULL
)
end;

IF NOT EXISTS ( SELECT  *
                FROM    sys.indexes
                WHERE   name = 'IDX_RATING_UID' ) 
begin
CREATE INDEX IDX_RATING_UID ON "review" (u_id)
end;

IF NOT EXISTS ( SELECT  *
                FROM    sys.indexes
                WHERE   name = 'IDX_RATING_AID' ) 
begin
CREATE INDEX IDX_RATING_AID ON "review" (a_id)
end;

IF NOT EXISTS ( SELECT  *
                FROM    sys.objects
                WHERE   name = 'review_rating' ) 
begin 
CREATE TABLE "review_rating" (
  u_id int NOT NULL REFERENCES "user" (u_id),
  a_id int NOT NULL,
  rating int NOT NULL,
  status int NOT NULL,
  creation_date datetime DEFAULT NULL,
  last_mod_date datetime DEFAULT NULL,
  type int DEFAULT NULL,
  vertical_id int DEFAULT NULL
)
end;

IF NOT EXISTS ( SELECT  *
                FROM    sys.indexes
                WHERE   name = 'IDX_REVIEW_RATING_UID' ) 
begin
CREATE INDEX IDX_REVIEW_RATING_UID ON review_rating (u_id)
end;

IF NOT EXISTS ( SELECT  *
                FROM    sys.indexes
                WHERE   name = 'IDX_REVIEW_RATING_AID' ) 
begin
CREATE INDEX IDX_REVIEW_RATING_AID ON review_rating (a_id)
end;

IF NOT EXISTS ( SELECT  *
                FROM    sys.objects
                WHERE   name = 'trust' ) 
begin 
CREATE TABLE "trust" (
  source_u_id int NOT NULL REFERENCES "user" (u_id),
  target_u_id int NOT NULL REFERENCES "user" (u_id),
  trust int NOT NULL,
  creation_date datetime DEFAULT NULL
)
end;

Delete from "review";
Delete from "review_rating";
Delete from "trust";
Delete from "item";
Delete from "user";
