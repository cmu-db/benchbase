                ...
                try {
                    ...
                    if (!isConnectionErrorException && SQLUtil.isConnectionOK(conn)) {
                        ...
                        if (conn.isReadOnly()) {
                            ...
                            LOG.debug(String.format("Won't attempt a rollback since the SQL connection looks read-only during [%s]... current retry attempt [%d], max retry attempts [%d], sql state [%s], error code [%d].", transactionType, retryCount, maxRetryCount, ex.getSQLState(), ex.getErrorCode()), ex);
                            try {
                                conn.close();
                            }
                            catch (SQLException ex2) {
