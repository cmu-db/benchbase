...
public class TimeseriesWorker extends Worker<TimeseriesBenchmark> {
    ...
    public void execGetSessionRange(Connection conn) throws SQLException {
        int session_low = rng().nextInt(this.getBenchmarkModule().num_sessions);
        int session_high =  session_low + rng().nextInt(5) * this.getBenchmarkModule().num_sources;
        int source_id = session_low % this.getBenchmarkModule().num_sources;
