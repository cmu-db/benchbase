...
public final class SEATSLoader extends Loader<SEATSBenchmark> {
  ...
  /** Store a list of FlightIds and the number of seats remaining for a particular flight. */
  ...
  protected void loadHistograms() {
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          String.format(
              ...
              SEATSConstants.HISTOGRAM_DATA_FILES.length, this.profile.airline_data_dir));
    }
    ...
    for (String histogramName : SEATSConstants.HISTOGRAM_DATA_FILES) {
      ...
      try {
        ...
        if (histogramName.equals(SEATSConstants.HISTOGRAM_FLIGHTS_PER_AIRPORT)) {
          Map<String, Histogram<String>> m =
              SEATSHistogramUtil.loadAirportFlights(this.profile.airline_data_dir);
          ...
          hist =
              SEATSHistogramUtil.loadHistogram(histogramName, this.profile.airline_data_dir, true);
        }
      } catch (Exception ex) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            String.format(
                "Loaded histogram '%s' [sampleCount=%d, valueCount=%d]",
  ...
  /**
   * The fixed tables are those that are generated from the static data files The number of tuples
   * in these tables will not change based on the scale factor.
   *
   * @param conn
   * @param table_name
   */
  protected void loadFixedTable(Connection conn, String table_name) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to load data files for fixed-sized table '" + table_name + "'", ex);
    }
  }

  /**
   * The scaling tables are things that we will scale the number of tuples based on the given
   * scaling factor at runtime
   *
   * @param conn
   * @param table_name
   */
  protected void loadScalingTable(Connection conn, String table_name) {
    try {
      ...
      throw new RuntimeException(
          "Failed to load data files for scaling-sized table '" + table_name + "'", ex);
    }
  }
  ...
  public void loadTable(
      Connection conn, Table catalog_tbl, Iterable<Object[]> iterable, int batch_size) {
    // Special Case: Airport Locations
    final boolean is_airport =
        catalog_tbl.getName().equalsIgnoreCase(SEATSConstants.TABLENAME_AIRPORT);

    if (LOG.isDebugEnabled()) {
      LOG.debug(
          String.format(
              "Generating new records for table %s [batchSize=%d]",
              catalog_tbl.getName().toLowerCase(), batch_size));
    }
      ...
      for (Object[] tuple : iterable) {
        ...
        if (is_airport) {
          ...
          if (!this.profile.hasFlights((String) tuple[col_code_idx])) {
            if (LOG.isTraceEnabled()) {
              LOG.trace(
                  String.format(
                      "Skipping AIRPORT '%s' because it does not have any flights",
                      tuple[col_code_idx]));
            }
            continue;
          }
          ...
          if (tuple[col_lat_idx] instanceof Float) {
            coords =
                Pair.of(
                    Double.valueOf(tuple[col_lat_idx].toString()),
                    Double.valueOf(tuple[col_lon_idx].toString()));
          } else {
          ...
          if (code.length() > 0) {
            ...
            if (LOG.isTraceEnabled()) {
              LOG.trace(
                  String.format(
                      "Mapping %s '%s' -> %s '%d'",
                      from_column.getName().toLowerCase(),
                      code,
                      to_column.getName().toLowerCase(),
                      id));
            }
        ...
        for (int i = 0; i < tuple.length; i++) {
          try {
            ...
            throw new RuntimeException(
                "Failed to set value for " + catalog_tbl.getColumn(i).getName().toLowerCase(), ex);
          }
        }
        ...
        if (++row_batch >= batch_size) {
          LOG.trace(
              String.format(
                  "Loading %s batch [total=%d]", catalog_tbl.getName().toLowerCase(), row_idx));
    ...
    LOG.debug(
        String.format(
            "Finished loading all %d tuples for %s", row_idx, catalog_tbl.getName().toLowerCase()));
  }
  ...
  protected Iterable<Object[]> getScalingIterable(Table catalog_tbl) {
    ...
    // Airport Distance
      ...
      int max_distance =
          Integer.MAX_VALUE; // SEATSConstants.DISTANCES[SEATSConstants.DISTANCES.length
      // - 1];
    ...
    // Reservations
      ...
      long total =
          Math.round(
              (SEATSConstants.FLIGHTS_PER_DAY_MIN + SEATSConstants.FLIGHTS_PER_DAY_MAX)
                  / 2d
                  * scaleFactor);
  ...
  /**
   * Base Iterable implementation for scaling tables Sub-classes implement the specialValue() method
   * to generate values of a specific type instead of just using the random data generators
   */
  protected abstract class ScalingDataIterable implements Iterable<Object[]> {
     ...
     * @param catalog_tbl
     * @param total
     * @param special_columns The offsets of the columns that we will invoke specialValue() to get
     *     their values
     * @throws Exception
     */
        ...
        public Object[] next() {
          ...
          for (int i = 0; i < ScalingDataIterable.this.data.length; i++) {
            ...
            if (ScalingDataIterable.this.special[i]) {
              ScalingDataIterable.this.data[i] =
                  ScalingDataIterable.this.specialValue(ScalingDataIterable.this.last_id, i);
  ...
  protected class CustomerIterable extends ScalingDataIterable {
    ...
    public CustomerIterable(Table catalog_tbl, long total) {
      ...
      Histogram<String> histogram =
          SEATSLoader.this.profile.getHistogram(SEATSConstants.HISTOGRAM_FLIGHTS_PER_AIRPORT);
        ...
        case (0):
          {
            ...
            int next_customer_id =
                SEATSLoader.this.profile.incrementAirportCustomerCount(airport_id);
            ...
            if (LOG.isTraceEnabled()) {
              LOG.trace(
                  ...
                  value,
                  this.airport_code,
                  SEATSLoader.this.profile.getCustomerIdCount(airport_id));
            }
            break;
          }
          // CUSTOMER ID STR
    ...
    protected void callbackFinished() {
      if (LOG.isTraceEnabled()) {
        ...
        LOG.trace(
            String.format(
                "Customer Local Airports Histogram [valueCount=%d, sampleCount=%d]\n%s",
                h.getValueCount(), h.getSampleCount(), h));
      }
    }
  }
  ...
  protected class FrequentFlyerIterable extends ScalingDataIterable {
    ...
    public FrequentFlyerIterable(Table catalog_tbl, long num_customers) {
      ...
      this.customer_id_iterator =
          new CustomerIdIterable(SEATSLoader.this.profile.airport_max_customer_id).iterator();
      ...
      this.airline_rand =
          new FlatHistogram<>(SEATSLoader.this.rng, SEATSLoader.this.flights_per_airline);
      if (LOG.isTraceEnabled()) {
      ...
      long max_per_customer =
          Math.min(
              Math.round(
                  SEATSConstants.CUSTOMER_NUM_FREQUENTFLYERS_MAX
                      * Math.max(1, SEATSLoader.this.scaleFactor)),
              SEATSLoader.this.flights_per_airline.getValueCount());
      Zipf ff_zipf =
          new Zipf(
              SEATSLoader.this.rng,
            ...
            while (this.customer_idx < this.ff_per_customer.length
              ...
              if (LOG.isTraceEnabled()) {
                LOG.trace(
                    String.format(
                        "CUSTOMER IDX: %d / %d",
                        this.customer_idx, SEATSLoader.this.profile.getCustomerIdCount()));
              }
    ...
    protected void callbackFinished() {
      if (LOG.isTraceEnabled()) {
        ...
        LOG.trace(
            String.format(
                "Airline Flights Histogram [valueCount=%d, sampleCount=%d]\n%s",
                h.getValueCount(), h.getSampleCount(), h));
      }
    }
  }
  ...
  protected class AirportDistanceIterable extends ScalingDataIterable {
    ...
    /**
     * Find the next two airports that are within our max_distance limit. We keep track of where we
     * were in the inner loop using last_inner_ctr
     */
    @Override
    protected boolean hasNext() {
  ...
  protected class FlightIterable extends ScalingDataIterable {
    ...
    public FlightIterable(Table catalog_tbl, int days_past, int days_future) {
      ...
      for (String airport_code : histogram.values()) {
        ...
        this.flights_per_airport.put(
            airport_code, new FlatHistogram<>(SEATSLoader.this.rng, histogram));
      }

      // Flights Per Departure Time
      histogram =
          SEATSLoader.this.profile.getHistogram(SEATSConstants.HISTOGRAM_FLIGHTS_PER_DEPART_TIMES);
      ...
      for (long t = this.today.getTime() - (days_past * SEATSConstants.MILLISECONDS_PER_DAY);
          t < this.today.getTime();
          t += SEATSConstants.MILLISECONDS_PER_DAY) {
      ...
      for (long t = this.today.getTime(),
              last_date =
                  this.today.getTime() + (days_future * SEATSConstants.MILLISECONDS_PER_DAY);
          t <= last_date;
          t += SEATSConstants.MILLISECONDS_PER_DAY) {
        Timestamp timestamp = new Timestamp(t);
        int num_flights = gaussian.nextInt();
    ...
    private Timestamp convertTimeString(Timestamp base_date, String code) {
      ...
      long offset =
          (hour * 60 * SEATSConstants.MILLISECONDS_PER_MINUTE)
    ...
    private void populate(Timestamp date) {
      ...
      this.arrive_time =
          SEATSLoader.this.calculateArrivalTime(
        ...
        case (0):
          {
            ...
            while (true) {
              ...
              this.flight_id =
                  new FlightId(
                      this.airline_id,
                      SEATSLoader.this.profile.getAirportId(this.depart_airport),
                      SEATSLoader.this.profile.getAirportId(this.arrive_airport),
                      this.start_date,
                      this.depart_time);
            ...
            if (LOG.isTraceEnabled()) {
              LOG.trace(
                  String.format(
                      "%s [remaining=%d, dayIdx=%d]", this.flight_id, remaining, this.day_idx));
            }
  ...
  protected class ReservationIterable extends ScalingDataIterable {
    private final RandomDistribution.Flat prices =
        new RandomDistribution.Flat(
            SEATSLoader.this.rng,
            SEATSConstants.RESERVATION_PRICE_MIN,
            SEATSConstants.RESERVATION_PRICE_MAX);

    /**
     * For each airport id, store a list of ReturnFlight objects that represent customers that need
     * return flights back to their home airport ArriveAirportId -> ReturnFlights
     */
     ...
     */
    private final Gaussian rand_returns =
        new Gaussian(
            SEATSLoader.this.rng,
    ...
    private void generateData() throws Exception {
      ...
      for (FlightId flight_id : SEATSLoader.this.getFlightIds()) {
        ...
        Timestamp depart_time =
            flight_id.getDepartDateAsTimestamp(SEATSLoader.this.profile.getFlightStartDate());
        ...
        int booked_seats =
            SEATSConstants.FLIGHTS_NUM_SEATS - SEATSLoader.this.getFlightRemainingSeats(flight_id);
        ...
        if (LOG.isTraceEnabled()) {
          ...
          m.put(
              ...
              String.format(
                  "%s / %s",
                  SEATSLoader.this.profile.getAirportCode(depart_airport_id), depart_time));
          m.put(
              "Arrival",
              String.format(
                  "%s / %s",
                  SEATSLoader.this.profile.getAirportCode(arrive_airport_id), arrive_time));
          m.put("Booked Seats", booked_seats);
          m.put(
              String.format("Returning Customers[%d]", returning_customers.size()),
              StringUtil.join("\n", returning_customers));
        ...
        for (int seatnum = 0; seatnum < booked_seats; seatnum++) {
          ...
          Integer airport_customer_cnt =
              SEATSLoader.this.profile.getCustomerIdCount(depart_airport_id);
          boolean local_customer =
              airport_customer_cnt != null && (flight_customer_ids.size() < airport_customer_cnt);
          int tries = 2000;
          ...
          if (return_flight != null) {
            if (LOG.isTraceEnabled()) {
              LOG.trace(
                  "Booked return flight: {} [remaining={}]",
                  return_flight,
            ...
            if (SEATSLoader.this.rng.nextInt(100) < SEATSConstants.PROB_SINGLE_FLIGHT_RESERVATION) {
              ...
              return_flight =
                  new ReturnFlight(customer_id, depart_airport_id, depart_time, return_days);
          ...
          if (LOG.isTraceEnabled()) {
            LOG.trace(
                String.format(
                    "New reservation ready. Adding to queue! [queueSize=%d]", this.queue.size()));
          }
    ...
    /**
     * Return a list of the customers that need to return to their original location on this
     * particular flight.
     *
     * @param flight_id
     * @return
        ...
        case (2):
          {
            ...
            if (SEATSLoader.this.profile.getReservationUpcomingOffset() == null
                && flight_id.isUpcoming(
                    SEATSLoader.this.profile.getFlightStartDate(),
                    SEATSLoader.this.profile.getFlightPastDays())) {
  ...
  public boolean addFlightId(FlightId flight_id) {
    ...
    if (this.profile.flight_upcoming_offset == null
        && this.profile.flight_upcoming_date.compareTo(
                flight_id.getDepartDateAsTimestamp(this.profile.flight_start_date))
            < 0) {
  ...
  /** Decrement the number of available seats for a flight and return the total amount remaining */
  ...
  /**
   * For the current depart+arrive airport destinations, calculate the estimated flight time and
   * then add the to the departure time in order to come up with the expected arrival time.
   *
   ...
   * @return
   */
  public Timestamp calculateArrivalTime(
      String depart_airport, String arrive_airport, Timestamp depart_time) {
